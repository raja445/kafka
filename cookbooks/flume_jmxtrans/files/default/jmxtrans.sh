...
DIRECT_MEMORY_SIZE=${DIRECT_MEMORY_SIZE:-"128"}
...
GC_OPTS=${GC_OPTS:-"-Xms${XMS_HEAP_SIZE}M -Xmx${HEAP_SIZE}M -XX:+UseConcMarkSweepGC -XX:MaxDirectMemorySize=${DIRECT_MEMORY_SIZE}m -XX:NewRatio=${NEW_RATIO} -XX:NewSize=${NEW_SIZE}m -XX:MaxNewSize=${NEW_SIZE}m  -XX:GCTimeRatio=9 -XX:PermSize=${PERM_SIZE}m -XX:MaxPermSize=${MAX_PERM_SIZE}m -XX:+UseTLAB -XX:CMSInitiatingOccupancyFraction=${IO_FRACTION} -XX:+CMSIncrementalMode -XX:+CMSIncrementalPacing -XX:ParallelGCThreads=${CPU_CORES} -Dsun.rmi.dgc.server.gcInterval=28800000 -Dsun.rmi.dgc.client.gcInterval=28800000  -Xloggc:${LOG_DIR}/gc_jmxtrans.log"}
